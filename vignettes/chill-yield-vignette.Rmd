...
author: "Cory Whitney, Katja Schiffers"
date: "`r Sys.Date()`"
output:
...
`chillkernelslicerange()` The chill portion intervals, optimized interquartile ranges shown in `chillviolin()` can be used as ranges to take a slice of the density kernel `chillkernel()`.

```{r, fig.width = 5}
pasitR::chillkernelslicerange(chill, yield)
```
